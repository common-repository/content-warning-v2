...
if ( !class_exists( 'AGY_Settings' ) ) {
    class AGY_Settings
    {
        ...
        public function admin_scripts()
        {
            ...
            if ( 'woocommerce_page_agy_settings' === $screen->base ) {
                // jQuery is needed.
                ...
                wp_enqueue_script(
                    ...
                    AGY_URL . '/assets/admin/wp-color-picker-alpha.min.js',
                    array(),
                    AGY_VERSION,
                    true
                );
        ...
        public function callback_text( $args )
        {
            ...
            $html = sprintf(
                '<input type="%1$s" class="%2$s-text ' . $args['class'] . ' ' . $args['premium'] . '" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"placeholder="%6$s"/>',
                $type,
                $size,
                $args['section'],
                $args['id'],
        ...
        public function callback_select( $args )
        {
            ...
            $size = ( isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular' );
            $html = sprintf(
                '<select class="%1$s ' . $args['premium'] . '" name="%2$s[%3$s]" id="%2$s[%3$s]">',
                $size,
                $args['section'],
                $args['id']
            );
        ...
        public function callback_code( $args )
        {
            ...
            $size = ( isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular' );
            $html = sprintf(
                '<textarea rows="5" cols="55" class="%1$s-text code-textarea" id="%2$s[%3$s]" name="%2$s[%3$s]">%4$s</textarea>',
                $size,
                $args['section'],
                $args['id'],
                $value
        ...
        public function callback_addon( $args )
        {
            ...
            $html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
            $html .= sprintf(
                '<input type="checkbox" class="toggle-checkbox" id="wposa-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />',
                $args['section'],
                $args['id'],
                checked( $value, 'on', false )
            );
        ...
        public function callback_file( $args )
        {
            ...
            $html = sprintf(
                '<input type="text" class="%1$s-text agy-url" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>',
                $size,
                $args['section'],
                $args['id'],
                $value
            );
            $html .= '<input type="button" class="button agy-browse" value="' . $label . '" />';
            $html .= $this->get_field_description( $args );
            echo  $html ;
        }
        ...
        public function callback_image( $args )
        {
            ...
            $html .= '<input type="button" class="button agy-browse" value="' . $label . '" />';
            $html .= $this->get_field_description( $args );
            $html .= '<p class="agy-image-preview"><img src=""/></p>';
            echo  $html ;
        }
        ...
        public function callback_color( $args )
        {
            ...
            $size = ( isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular' );
            $html = sprintf(
                '<input type="text" class="%1$s-text color-picker" data-alpha="true" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" data-default-color="%5$s" placeholder="%6$s" />',
                $size,
                $args['section'],
                $args['id'],
                $value,
        ...
        public function callback_separator( $args )
        {
            ...
            $html = '';
            $html .= '<div class="agy-settings-separator"></div>';
            echo  $html ;
        }
        ...
        {
            ?>
			<div class="agy-admin-header">
				<div class="logo"><img src="<?php 
            echo  AGY_URL . '/assets/admin/agy-logo.svg' ;
            ?>" /></div>
				<div class="info-links">
					<?php 
            ?>
						<a href="https://patrickposner.dev/plugins/agy/" target="_blank">Go Pro</a>
						<a href="https://patrickposner.dev/docs/agy" target="_blank">Documentation</a>
						<a href="https://wordpress.org/plugins/content-warning-v2/" target="_blank">Support</a>		
					<?php 
            ?>
				</div>
			</div>
			<div class="wrap agy-admin">
				<?php 
        ...
        public function script()
        {
			...
			<script>
				jQuery(document).ready(function ($) {
					...
					$('.agy-browse').on('click', function (event) {
			...
			<style type="text/css">
				...
				.agy-image-preview img {
					height: auto;
					max-width: 70px;
				}

				.agy-settings-separator {
					background: #ccc;
					border: 0;
					color: #ccc;
					height: 1px;
